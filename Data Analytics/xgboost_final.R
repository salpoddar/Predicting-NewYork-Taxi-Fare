...
train=read_csv('C:\\train.csv',col_types =list(
  key=col_character(),
...
#create new columns and add to dataframe
...
#calculate locations of famous landmarks
#jfk
jfk_lat<-40.6413
jfk_long<--73.7781
...
#calculate distance from famous landmarks
...
index=createDataPartition(train$year,p=0.8,list=FALSE)
dtrain_3=train[index,]
dtest_3=train[-index,]
...
importance_matrix <- xgb.importance(colnames(dtrain_4_matrix), model = m_xgb)
